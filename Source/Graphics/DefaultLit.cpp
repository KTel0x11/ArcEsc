#include "Misc.h"
#include "Graphics/DefaultLit.h"

DefaultLit::DefaultLit(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\DefaultLitVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\DefaultLitPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void DefaultLit::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
...
// 描画
void DefaultLit::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void DefaultLit::End(ID3D11DeviceContext* dc)
{
