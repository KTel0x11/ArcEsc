...
Graphics::Graphics(HWND hWnd)
{
	...
	// デバイス＆スワップチェーンの生成
	{
		...
		hr = D3D11CreateDeviceAndSwapChain(
			adapter,						// どのビデオアダプタを使用するか？既定ならばnullptrで、IDXGIAdapterのアドレスを渡す。
			D3D_DRIVER_TYPE_UNKNOWN,		// ドライバのタイプを渡す。D3D_DRIVER_TYPE_HARDWARE 以外は基本的にソフトウェア実装で、特別なことをする場合に用いる。
	...
	// シェーダー
	{
		...
		shaders[static_cast<int>(ShaderId::DefaultLit)] = std::make_unique<DefaultLit>(device.Get());
