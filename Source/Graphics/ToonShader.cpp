...
ToonShader::ToonShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ToonVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ToonPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...

	{
		...
		desc.Usage = D3D11_USAGE_DEFAULT;
		desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
		desc.ByteWidth = sizeof(CbToonLighting);

		HRESULT hr = device->CreateBuffer(&desc, nullptr, toonLightingConstantBuffer.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
...
void ToonShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
	...
	CbToonLighting cbToonLighting = { {0.2f, 0.5f, 0.8f, 1.0f} }; // ステップ閾値設定
	dc->UpdateSubresource(toonLightingConstantBuffer.Get(), 0, nullptr, &cbToonLighting, 0, 0);
	...
	ID3D11Buffer* constantBuffers[] =
	{
		...
		toonLightingConstantBuffer.Get() // トゥーン用バッファ
	};
...
// 描画
void ToonShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
