...
#define debug_new new(_NORMAL_BLOCK,__FILE__,__LINE__)
 ...
 const int AxisX_MAX = 10;
 const int AxisY_MAX = 10;
...
private :
	...
	bool MoveAnimation(DirectX::XMFLOAT2 startPos, DirectX::XMFLOAT2 endPos, float moveSpeed);

	void ResetRoom(DirectX::XMINT3 Axis);

	void DrawDebugGUI();
	//部屋入手処理
	void GetRoom(int Type);
	...
	int determineRoomAngle(const DirectX::XMINT3& axis) {
		...
		if (axis.x == 9) return 0;
		if (axis.y == 0) return 3;
		if (axis.y == 9) return 1;
		return rand() % 4;
	}
...
private:
	...
	//地図のスプライト関係
		...
		DirectX::XMFLOAT2 spriteMapPos = { 270,25 };
		float spriteMapSize = 700.0f;
	...
	//枠スプライト関係
		...
		DirectX::XMFLOAT2 spriteFrameSize = { 56,56 };
	//設計図の木枠スプライト関係
	...
	//コイン枠スプライト関係
		...
		Room* MapRoom[10][10];//地図の部屋情報

		int rnd = 0;//乱数用


		DirectX::XMFLOAT2 spriteBlockPos = { 350, 90 };
		DirectX::XMFLOAT2 spriteBlockSize = { 50,50 };
		float color[10][10]{};
