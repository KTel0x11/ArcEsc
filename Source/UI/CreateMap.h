...
const int Axis_MAX = 5;
 ...
 private:
	 ...
	 //メニュー処理
	 void StartMenu(float elapsedTime);//メニューの初期化
	 void EndMenu(float elapsedTime);//メニューの終了化
	 void SelectMenu(float elapsedTime);//メニューの選択
	 void MenuRender(ID3D11DeviceContext* dc);//メニューの描画

	 //部屋制作の処理
	 void StartMake(float elapsedTime);//部屋制作の初期化
	 void EndMake(float elapsedTime);//部屋制作の終了化
	 void MakeRoom();//部屋制作の処理
	 void MakeRender(ID3D11DeviceContext* dc);//部屋制作の描画
	 void RoomRotate();//部屋の回転
	 bool SelectBluePrint();//部屋の選択

	 //ショップの処理
	 void StartShop(float elapsedTime);//ショップの初期化
	 void EndShop(float elapsedTime);//ショップの終了化
	 void ShopMenu(float elapsedTime);//ショップの処理
	 void ShopRender(ID3D11DeviceContext* dc);//ショップの描画
 ...
 private:
	 ...
	 float spriteBlockInterval = 6.0f;
